...
const char* serverName = "https://lgbtwzarrwsqyizzkbro.supabase.co/rest/v1/details";
...
void setup() {
  ...
  Serial.println("");
  Serial.println("GPS Serial Monitor Initialized");
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    std::string jsonString = "{\"bus_id\":1,\"lat\":" + std::to_string(gps.location.lat()) + ",\"long\":" + std::to_string(gps.location.lng()) + ",\"capacity\":" + std::to_string(passengerCount) + "}";
